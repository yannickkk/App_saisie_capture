...
print(time)
  ...
  sup_Ligne_prelev = observeEvent(input$sup_prelev, {
    if (!is.null(input$tableprelevement_rows_selected)) {
      ...
      id_prel_cont = dbGetQuery(con, paste0("SELECT sac_id from lu_tables.tr_samples_contenant_sac where sac_contenant = '",input$condi,"' AND sac_sat_id = '",id_prel_type,"'"))
...
observe({
 ...
 if(((input$estNouvelAnimal == 'oui') || (input$estNouvelAnimal == 'non' && input$identifie == 'non')) && input$nAnimal!="") {
            ...
            if (faon) {
              if   (input$idTagOrD !=""){
                cap_bague = paste0("F", input$idTagOrD, "_", str_sub(annee_suivie, -2)) } else{cap_bague = paste0("F", input$idTagOrG, "_", str_sub(annee_suivie, -2))}
            }
            if (faon) {
                if   (input$idTagOrD !=""){
                  cap_bague = paste0(input$idTagOrD, "_", str_sub(annee_suivie, -2)) } else{cap_bague = paste0(input$idTagOrG, "_", str_sub(annee_suivie, -2))}
            }
            ani_id<- dbGetQuery(con, paste0("select ani_id from public.t_animal_ani where ani_etiq= '",input$nAnimal2,"'"))[1,1]
            cap_id<- dbGetQuery(con, "SELECT cap_id FROM t_capture_cap WHERE cap_ani_id = ",ani_id,"")
              
            if (exists("cap_id")) {
####plantage cap_id null              
              print(paste0("cap_id :",cap_id))
                req<-paste0("UPDATE public.t_capture_cap SET (cap_bague) = ('",cap_bague,"') where  cap_id= ",cap_id," ")
 ...
 if(input$estNouvelAnimal == 'non' && input$identifie == 'oui' && input$nAnimal2!="") {
            ...
            print(save1[,"cap_tag_droit"])
            
            if (faon) {
              if   (save1[,"cap_tag_droit"] !=""){
                cap_bague = paste0("F", save1[,"cap_tag_droit"], "_", str_sub(annee_suivie, -2)) } else{cap_bague = paste0("F", save1[,"cap_tag_gauche"], "_", str_sub(annee_suivie, -2))}
            } 
            if (faon) {
              if   (input$idTagOrD !=""){
                cap_bague = paste0(save1[,"cap_tag_droit"], "_", str_sub(annee_suivie, -2)) } else{cap_bague = paste0(save1[,"cap_tag_gauche"], "_", str_sub(annee_suivie, -2))}
            }
            ani_id<- dbGetQuery(con, paste0("select ani_id from public.t_animal_ani where ani_etiq= '",input$nAnimal2,"'"))[1,1]
            cap_id<- dbGetQuery(con, "SELECT cap_id FROM t_capture_cap WHERE cap_ani_id = ",ani_id,"")
...
##################           BASE DE DONNEES                            ####
        ...
        if (input$estNouvelAnimal == 'oui') {
          observeEvent(input$cre_ind,{
                  #####il faut creer une alerte si les nom est trop court, trop long ou si il manque le sexe quand on appuit sur le bouton créer
              if (input$nAnimal != "" && nchar(as.character(input$nAnimal)) >= 4 && nchar(as.character(input$nAnimal)) < 6 && !is.null(input$sexe)){
        ...
        ######creation d''une entree dans t_capture_cap
           ...
           if (input$idRFID != "" && exists("cap_id") && !is.null(cap_id)) {
       ...
       ####### recapture d''un animal identifié ####
          ...
          if (input$nAnimal2 != "" && input$idSite2 != "" && as.character(input$date_caract) != "" && !is.null(input$sexe)){
            ...
            if (input$newTagG == TRUE && input$idTagOrG3 != "" && exists("cap_id") && !is.null(cap_id)) {
            ...
            if (input$idTagOrD2!= "" && input$newTagG == FALSE && exists("cap_id") && !is.null(cap_id)){
              req<-paste0("UPDATE public.t_capture_cap SET (cap_tag_gauche, cap_tag_gauche_metal) = ('",input$idTagOrD2,"',",input$metal_tag_d2,") where  cap_id= '",cap_id,"' ")
            ...
            if (input$newTagD == TRUE && input$idTagOrD3 != "" && exists("cap_id") && !is.null(cap_id)) {
  
            req<-paste0("UPDATE public.t_capture_cap SET (cap_tag_gauche, cap_tag_gauche_metal, cap_bague) = ('",input$idTagOrD3,"',",input$metal_tag_d3,") where  cap_id= ",cap_id," ")
            ...
            if (input$nAnimal2!="" && exists("cap_id") && !is.null(cap_id) && exists("annee")) {
              cap_pertinent = dbGetQuery(con,paste0("select cap_annee_suivi from public.t_capture_cap, public.t_animal_ani where cap_ani_id=ani_id and ani_etiq = '",input$nAnimal2,"' order by cap_annee_suivi DESC"))
            ...
            if (input$newRFIDbox == FALSE && input$idRFID2 != "" && exists("cap_id") && !is.null(cap_id)) {
            ...
            if (input$newRFIDbox == TRUE && input$idRFID_new != "" && exists("cap_id") && !is.null(cap_id)) {
       ...
       ####### recapture d''un animal non identifie ####
          ...
          if (input$estNouvelAnimal == 'non' && input$identifie == 'non') {
            ...
            if (input$nAnimal != "" && nchar(as.character(input$nAnimal)) >= 4 && nchar(as.character(input$nAnimal)) < 6 && input$idSite != "" && as.character(input$date_caract) != "" && !is.null(input$sexe)){
                  ...
                  req<- paste0("INSERT INTO public.t_capture_cap(cap_ani_id, cap_sit_id, cap_date, cap_annee_suivi, cap_faon, cap_poids, cap_num_sabot, cap_bague, cap_tag_droit, cap_tag_droit_metal) 
                    ...
                    cap_id<-dbGetQuery(con,req)}})######fin de la nouvelle entree dans t_capture_cap
                  
                
                if (input$idTagOrD != "" && exists("cap_id") && !is.null(cap_id)) {
  
                  req<-paste0("UPDATE public.t_capture_cap SET (cap_tag_droit, cap_tag_droit_metal,cap_bague) = ('",input$idTagOrD,"',",input$metal_tag_d,") where  cap_id= ",cap_id," ")
                ...
                if (input$idTagOrG != "" && exists("cap_id") && !is.null(cap_id)) {
                  ...
                  req<-paste0("UPDATE public.t_capture_cap SET (cap_tag_gauche, cap_tag_gauche_metal, cap_bague) = ('",input$idTagOrG,"',",input$metal_tag_g,") where  cap_id= ",cap_id," ")
          ...
          if (input$idSite != ""&& exists("cap_id") && !is.null(cap_id)) {
            find_site_id <- dbGetQuery(con, paste0("select sit_id from public.tr_site_capture_sit where sit_nom_court= '",input$idSite,"'"))[1,1]
          ...
          if (input$numSabot != "" && exists("cap_id") && !is.null(cap_id)) {
            ...
            if (input$age != "" && exists("cap_id") && !is.null(cap_id)) {
            ...
            if (!is.na(input$cirCou) && exists("cap_id") && !is.null(cap_id)) {
            ...
            if (!is.na(input$lPattArriere) && exists("cap_id") && !is.null(cap_id)) {
            ...
            if (input$diarrhee != "" && exists("cap_id") && !is.null(cap_id) || input$remarque_ani!= "" && exists("cap_id") && !is.null(cap_id) || input$liste_blessures!= "" && exists("cap_id") && !is.null(cap_id)) {
              
              if (input$diarrhee != "") {diarrhee = paste0("diarrhee/",input$diarrhee,"")} else {diarrhee = "diarrhee/"}
              if (input$remarque_ani != "") {reactive(input$remarque_ani <- sub("'","''",as.character(input$remarque_ani)))} 
            ...
            if (input$time != "" && exists("cap_id") && !is.null(cap_id)) {
              if (input$time2 != ""){
            ...
            if (input$Notation_euro_table != "" && exists("cap_id") && !is.null(cap_id)) {
            ...
            if (input$Notation_euro != "" && exists("cap_id") && !is.null(cap_id)) {
          ...
          if (!is.na(input$lBoisGauche) && input$sexe == 'M' && exists("cap_id") && !is.null(cap_id)) {
          ...
          if (!is.na(input$lBoisDroit) && input$sexe == 'M' && exists("cap_id") && !is.null(cap_id)) {
          ...
          if (input$etatBois != "" && !is.null(input$sexe) && input$sexe == 'M' && exists("cap_id") && !is.null(cap_id)) {
          ...
          if (input$lactation != ""  && !is.null(input$sexe) && input$sexe == 'F' && exists("cap_id") && !is.null(cap_id)) {
...
#              id_prel_cont = dbGetQuery(con, paste0("SELECT sac_id from lu_tables.tr_samples_contenant_sac where sac_contenant = '",prel_cont,"' AND sac_sat_id = '",id_prel_type,"'"))
